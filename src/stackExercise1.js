...
var StackExercise1 = function StackExercise1() {
  this.storage = "";
  //  console.log("min element = " + this.minElement);
  this.sortedElements = [];
};

StackExercise1.prototype.push = function(value) {
...
StackExercise1.prototype.pop = function() {
  const returnString = this.peek();
  this.storage = this.storage.slice(0, this.storage.lastIndexOf("-"));
  const indexToRemove = this.sortedElements.indexOf(returnString);
  this.sortedElements.splice(indexToRemove, 1);
  this.sortedElements.sort();
  return returnString;
};
// Time complexity:

StackExercise1.prototype.peek = function() {
  return this.storage.slice(this.storage.lastIndexOf("-") + 1);
};
// Time complexity:

StackExercise1.prototype.count = function() {
  return (this.storage.match(/\-/g) || []).length;
};
// Time complexity:

StackExercise1.prototype.min = function() {
  return this.sortedElements[0];
};
// Time complexity:

module.exports = StackExercise1;
